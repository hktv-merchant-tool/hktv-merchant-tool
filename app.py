 """
  ...
  from pptx_generator import generate_merchant_ppt_with_notes, generate_custom_ppt
  ...
  CATEGORY_DATA = {
      "糧油雜貨": {
          ...
          "優勢話術": "糧油雜貨是HKTVmall GMV最高的品類，家庭消費者忠誠度高，復購率達4.7x/季，加入我們立即享受流量紅利。",
      },
      "美容及健康產品": {
          "gmv佔比": "中高",
          "目標客群": "女性消費者、年輕白領",
          ...
          "優勢話術": "美容及健康產品在HKTVmall增長迅猛，平台用戶男女均衡，個人專屬價工具能精準觸及目標客戶，20%轉化率效果顯著。",
      },
      "寵物用品": {
          "gmv佔比": "中高",
          "目標客群": "寵物主人、家庭客戶",
      ...
      "電子電器產品": {
          ...
          "優勢話術": "蘇寧等大型品牌已在HKTVmall直播單晚創下近90萬GMV，電子電器產品極適合直播展示功能，搶佔高端客戶群。",
      },
      "其他": {
          "gmv佔比": "一般",
          "目標客群": "大眾消費者",
          "物流要求": "常溫",
          "熱門工具": ["個人專屬價", "直播頻道", "廣告投放"],
          "優勢話術": "HKTVmall 310萬+用戶覆蓋全港各類消費者，全方位營銷工具助您快速打開市場，3-5個工作天即可開店營運。",
      },
  }

  TONE_OPTIONS = ["誠懇專業型", "強勢說服型", "溫暖關懷型", "數據導向型", "簡潔利落型", "輕鬆友好型"]
  ...
  def build_system_prompt(merchant_name, category, pain_points, custom_prompt, tone, output_format):
      ...
      prompt_parts.append(f"【品類特徵】GMV佔比：{cat_data['gmv佔比']}｜目標客群：{cat_data['目標客群']}｜物流要求：{cat_data['物流要
  求']}")
  ...
  def generate_recruitment_content(merchant_name, category, pain_points, custom_prompt, tone, output_format):
      ...
      pain_texts = [PAIN_POINT_MAPPING[p] for p in pain_points if p in PAIN_POINT_MAPPING]
      openings = {
          "誠懇專業型": f"您好，{merchant_name}團隊，\n\n非常高興有機會向您介紹 HKTVmall — 香港 No.1
  網上購物平台，以及我們為您量身訂造的商戶合作方案。",
          "強勢說服型": f"{merchant_name}，您好！\n\n我必須直接告訴您：在香港電商市場，已經有超過310萬消費者在HKTVmall購物，而您的品
  牌還未入駐，這是一個不容忽視的商機。",
          "溫暖關懷型": f"您好呀，{merchant_name}！\n\n知道您一直在用心經營品牌，我們很希望能把 HKTVmall
  的資源帶給您，讓您的生意事半功倍。",
  ...
  def main():
      st.set_page_config(page_title="HKTVmall B2B 智慧招商工具", page_icon="🛒", layout="wide")
      st.markdown("""
      <style>
      @import url('https://fonts.googleapis.com/css2?family=Noto+Sans+TC:wght@300;400;500;700&display=swap');
      * { font-family: 'Noto Sans TC', 'PingFang TC', 'Microsoft JhengHei', sans-serif; }
      .hero-banner { background: linear-gradient(135deg, #106946 0%, #0D5238 45%, #E47D21 100%); padding: 2rem 2rem 1.5rem;
  border-radius: 16px; margin-bottom: 1.5rem; color: white; }
      ...
      .metric-card { background: #f8faf8; border-left: 4px solid #106946; padding: 0.8rem 1rem; border-radius: 8px; margin-bottom:
  0.5rem; font-size: 0.88rem; }
      .section-header { font-size: 0.75rem; font-weight: 700; text-transform: uppercase; letter-spacing: 1px; color: #106946; margin:
  1rem 0 0.5rem; border-bottom: 2px solid #E47D21; padding-bottom: 4px; }
      .output-box { background: #fafafa; border: 1.5px solid #e0e0e0; border-radius: 12px; padding: 1.5rem; font-size: 0.9rem;
  line-height: 1.7; white-space: pre-wrap; min-height: 400px; }
      div[data-testid="stMainBlockContainer"] { padding-top: 0; }
      .stButton>button { background: linear-gradient(135deg, #106946, #0D5238) !important; color: white !important; font-weight: 700
  !important; font-size: 1.05rem !important; padding: 0.6rem 2rem !important; border: 2px solid #E47D21 !important; border-radius:
  10px !important; width: 100% !important; transition: all 0.3s !important; }
      .stButton>button:hover { background: linear-gradient(135deg, #E47D21, #F5A623) !important; box-shadow: 0 4px 15px
  rgba(228,125,33,0.4) !important; }
      ...
      <div class="hero-banner">
          ...
          <div class="hero-sub">基於「2026 HKTVmall New Merchant Acquisition Deck [CHI]」| 內置310萬+用戶數據 |
  即時生成招商提案</div>
      </div>
      """, unsafe_allow_html=True)
      ...
      with col_left:
          ...
          merchant_desc = st.text_area("主要業務描述",
  placeholder="例如：永明凍肉有限公司是一家專營優質進口凍肉及本地鮮肉批發的貿易商，客戶主要為餐廳及酒店...")
          ...
          custom_prompt = st.text_area("在這裡輸入您的特殊指示",
  placeholder="例如：請用非常誠懇但強勢的語氣撰寫、強調我們能幫他清臨期百貨庫存的優勢、生成一封適合 WhatsApp 傳送的 200 字短訊息...",
  height=120, help="這段文字會附加到生成Prompt中，用於特殊風格或格式要求")
          uploaded_ppt = None
          if output_format == "上傳PPT並客製化":
              st.markdown("---")
              st.markdown('<div class="section-header">📤 上傳PPT模板</div>', unsafe_allow_html=True)
              uploaded_file = st.file_uploader("拖放或選擇PPT檔案（.pptx）", type=["pptx", "ppt"],
  help="上傳HKTVmall官方PPT或其他招商模板，系統會根據您輸入的商戶資料自動客製化")
              if uploaded_file is not None:
                  st.success(f"已上傳：{uploaded_file.name}（{uploaded_file.size / 1024:.1f} KB）")
                  uploaded_ppt = uploaded_file.getvalue()
      with col_right:
  ...
  mime="application/vnd.openxmlformats-officedocument.presentationml.presentation")
                      ...
                      with st.spinner("正在交叉匹配數據並生成提案..."):
                          result = generate_recruitment_content(merchant_name=merchant_name, category=category,
  pain_points=selected_pains, custom_prompt=custom_prompt, tone=tone, output_format=output_format)
                      st.markdown(f'<div class="output-box">{result}</div>', unsafe_allow_html=True)
                      st.download_button(label="📥 下載提案文字檔", data=result, file_name=f"HKTVmall_招商提案_{merchant_name}.txt",
  mime="text/plain")
          else:
